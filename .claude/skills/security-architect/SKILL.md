...
5. **Language-specific patterns to check** (apply whichever is relevant to the language in scope):
   ...
   - **Chrome Extension (JavaScript / MV3)**: content scripts setting `innerHTML` with scraped DOM data without sanitization, `chrome.storage` persisting PII or auth tokens in plaintext (use `chrome.storage.session` or encrypt), `externally_connectable` configured with `"matches": ["<all_urls>"]` allowing any origin to message the extension, missing origin validation on `chrome.runtime.onMessageExternal` handlers, overly broad host permissions in `manifest.json` (`<all_urls>` instead of specific domains), `fetch`/`XMLHttpRequest` in content scripts sending captured data to unvalidated URLs, hardcoded API endpoints or tokens in extension source, content scripts injecting form fields that harvest credentials, missing `content_security_policy` in manifest allowing unsafe-inline or unsafe-eval
